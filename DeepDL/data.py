...
class MyDataset(Dataset):
    def __init__(self, context, line, target):
        self.context = context
        self.line = line
        self.target = target
    def __getitem__(self, item):
        return self.context[item], self.line[item], self.target[item]
...
def fetch_dataloaders(params, corpus):
    ...
    for i in range(0, len(train_data), 5):
        if i + 5 > len(train_data):
            break
        ths_context = list(train_data[i]) + list(train_data[i+1]) + list(train_data[i+3]) + list(train_data[i+4])
        ths_line = list(train_data[i+2])
        ths_target = list(train_data[i+2])
        train_context.append(ths_context)
        train_line.append(ths_line)
        train_target.append(ths_target)
    train_context = np.array(train_context)
    train_line = np.array(train_line)
    ...
    for i in range(0, len(test_data), 5):
        if i + 5 > len(test_data):
            break
        ths_context = list(test_data[i]) + list(test_data[i+1]) + list(test_data[i+3]) + list(test_data[i+4])
        ths_line = list(test_data[i+2])
        ths_target = list(test_data[i+2])
        test_context.append(ths_context)
        test_line.append(ths_line)
        test_target.append(ths_target)
    test_context = np.array(test_context)
    test_line = np.array(test_line)
    test_target = np.array(test_target)


    train_dataset = MyDataset(train_context, train_line, train_target)
    test_dataset = MyDataset(test_context, test_line, test_target)
...
class Corpus(object):
    def __init__(self, path, train_path, test_path, max_length):
        ...
        self.train = self.tokenize_train(os.path.join(path, train_path), max_length)
        self.test = self.tokenize_test(os.path.join(path, test_path), max_length)
        ...
        with open(path, 'r', encoding="utf8") as f:
            ...
            for line in f:
                if line_num <= 4:
                    ...
                    for word in line:
                        if (word in self.dictionary.word_to_id):
                            ...
                            ids.append(self.dictionary.word_to_id[UNKNOWN_WORD])
                    # ids.append(corpus.dictionary.word_to_id[word])
                    if len(ids) < line_length - 1:
                        for i in range(0, line_length - len(ids) - 1):
                            ids.append(self.dictionary.word_to_id[EMPTY_WORD])

                    elif len(ids) >= line_length:
                        ids = ids[0:line_length - 1]

                    ids.append(self.dictionary.word_to_id[END_LINE])
                    idss.append(ids)
                    line_num += 1
                ...
                if line_num == 5:
                    ...
                    ids.append(self.dictionary.word_to_id[END_SENT])
                    idss.append(ids)
                    line_num = 1
            idss = np.array(idss)
        return idss
