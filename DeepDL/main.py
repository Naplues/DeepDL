...
def train_model(params, train_data):
    ...
    for epoch in range(1, params.epochs + 1):
        ...
        for i, (context, line, target) in enumerate(tqdm(train_data)):
            ...
            context = torch.cuda.LongTensor(context.to(params.device))
            line = torch.cuda.LongTensor(line.to(params.device))
            target = torch.cuda.LongTensor(target.to(params.device))
            optimizer.zero_grad()
            output = model(line, context, target)
...
def eval_model(params, test_data, all_csv_path, test_csv_path, corpus):
    ...
    entropy_list = []
    with torch.no_grad():
        for i, (context, line, target) in enumerate(tqdm(test_data)):
            context = torch.cuda.LongTensor(context.to(params.device))
            line = torch.cuda.LongTensor(line.to(params.device))
            target = torch.cuda.LongTensor(target.to(params.device))
            output = model(line, context, target)
...
def main():
    ...
    for project in projects:
        ...
        corpus = data.Corpus(params.data, train_path, test_path, params.line_length)
