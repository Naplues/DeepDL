...
class DeepDLModel(nn.Module):
    ...
    def forward(self, line_input, context_input, target):
        ...
        target_pos = self.pos_encoder(target_embed)

        context_output = self.context_encoder(context_output)
        line_output = self.line_encoder(line_output)
        mix_output = context_output.cat(line_output)

        attn_output = self.attn_layer(mix_output)

        decoder_output = self.decoder(target_pos, attn_output)
